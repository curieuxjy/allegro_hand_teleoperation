...
class RobotKinematicsDataset:
    ...
    def __getitem__(self, idx):
        ...
        for name in self.keypoint_names:
            ...
            # Verified that stacking occurs in order
    ...
    def export_robot_pointcloud(self, keypoint_names):
        # print("export_robot_pointcloud", keypoint_names)
        all_keypoint_data = []
        for keypoint_name in keypoint_names:
            # Verified that dict keys are specified in name order
            # print(keypoint_name)
            all_keypoint_data.append(self.keypoints[keypoint_name])
        return np.array(all_keypoint_data)

if __name__ == '__main__':
    dataset = RobotKinematicsDataset("../data/allegro_right.npz",["link_3.0_tip", "link_7.0_tip"])
    print(dataset[0])
