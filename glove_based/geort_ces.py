...
"""
GeoRT CES Demo Deployer

Simplified deployment script with hardcoded checkpoints for CES demo.
Based on geort_allegro_deploy_pinch.py with pinch gesture detection.
"""
...
# Hardcoded checkpoint paths for CES demo
RIGHT_CKPT = "miller_right_1203_145107_allegro_right_2025-12-04_09-51-53_re"
LEFT_CKPT = "miller_left_1203_150345_allegro_left_2025-12-04_09-52-07_re"
...
def main():
    parser = argparse.ArgumentParser(
        description='GeoRT CES Demo - Deploy to Allegro hands with hardcoded checkpoints',
...
Example:
  ...
  %(prog)s --smoothing_alpha 0.7
    ...
    print("=" * 60)
    print("GeoRT CES Demo Deployer")
    print("=" * 60)
    print(f"Right checkpoint: {RIGHT_CKPT}")
    print(f"Left checkpoint:  {LEFT_CKPT}")
    print("=" * 60)
    ...
    # Load models from hardcoded checkpoint tags (always use best)
    print(f"[Loading] Right hand model: {RIGHT_CKPT}")
    right_model = load_model(RIGHT_CKPT, epoch='best')
    print(f"[Loading] Left hand model: {LEFT_CKPT}")
    left_model = load_model(LEFT_CKPT, epoch='best')
